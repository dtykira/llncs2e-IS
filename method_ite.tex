...
In order to investigate the clustering effect of iterative characteristics, we define iterative hulls as the set of iterative characteristics with the same input and output value and the same number of rounds. 

The procedure is given in Algorithm \ref{algo:find_ite_h_G}.
...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
        ...
        \Procedure {}{}
        \State $bwpr\leftarrow\infty$
        \For{each distinct $G$'s strong component with vertex set $V_{sc}$ applying Tarjan's algorithm in \cite{tarjan1972depth}}
        \State Let $\calH(\cdot,\cdot,\cdot)$ be an empty hash table. $\calH(u,v,d)\leftarrow c(u\rightarrow_d v),\forall u,v\in V_{sc},\forall$ existent $d$
        \For{$i\leftarrow 2:r$}
