...
\begin{algorithm}
	\caption{Compute $B_r$}
	\label{algo1}
	\begin{algorithmic}[1] %æ¯è?Œæ˜¾ç¤ºè?Œå·
		\Require $r$, parameters $r^f\leq r,r^b\leq r$, the round function $F$, $G^{IT}_{F,max\_asn}$
		\Ensure $B_r$
		\Procedure {ComputeB}{}
		\For{each $u\in V^{IT}_{F,max\_asn}$}
		\State $B^f_{u,0}\leftarrow 1,B^b_{u,0}\leftarrow 1$
		\EndFor
		\For{each $u\in V^{IT}_{F,max\_asn}$ and $i\leftarrow 1:r^f$}
		\State $BW\leftarrow B^f_{u,i-1}\times\max\limits_{a,b} \Prob^F(a,b)$
		\While{\Call{Search}{$u,i,0,F,BW$}}
		\State $BW\leftarrow BW\times 2^{-1}$
		\EndWhile
		\State $B^f_{u,i}\leftarrow BW$
		\EndFor
		\For{each $u\in V^{IT}_{F,max\_asn}$ and $i\leftarrow 1:r^b$}
		\State $BW\leftarrow B^b_{u,i-1}\times\max\limits_{a,b} \Prob^{F^{-1}}(a,b)$
		\While{\Call{Search}{$u,i,0,F^{-1},BW$}}
		\State $BW\leftarrow BW\times 2^{-1}$
		\EndWhile
		\State $B^b_{u,i}\leftarrow BW$
		\EndFor
...
\]
$w(h_{u,v,r})$ can be computed by dynamic programming.

Based on iterative trails, the largest probability (correlation) that a $r$-round differential (linear hull) can has $BC_r$ is
\[
	BC_r=\max\limits_{\substack{x,y\in \mathbb{F}_2^n\\u,v\in V^{IT}_{F,max\_asn}\\r_1+r_2+r_3=r\\0\leq r_1\leq r^b,0\leq r_3\leq r^f}} w^b_{x,u,r_1}\times w(h_{u,v,r_2})\times w^f_{v,y,r_3}
\]
