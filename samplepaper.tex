...
In this work, we propose a method of searching for iterative differential and linear distinguishers using a graph approach. Assume that the best distinguisher is dominated by iterative characteristics, we further propose a method of estimating EDPs and ELPs. 

We apply our methods to some S-bP symmetric primitives, including RECTANGLE, PRESENT, GIFT and so on. We find that iterative characteristics can always be found for them. By considering the clustering effect, we may find better iterative distinguishers. Using the knowledge of iterative characteristics, we estimate EDPs and ELPs and illustrate that the assumption of the dominance of iterative characteristics is reasonable. 
...
\input{method_ite.tex}
\input{result_ite.tex}
